        ...
        {activeMenu === "dashboard" && (
          <section>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
              ...
              {/* -----  MODIFIED LINE CHART BLOCK BELOW ----- */}
              <div className="bg-white shadow rounded-xl p-8 flex flex-col items-center">
                <h2 className="text-xl font-bold mb-3 text-gray-800">{t.regChartTitle}</h2>
                <ResponsiveContainer width="100%" height={250}>
                  {
                    regByDay.every(d => d.count === 0) ? (
                      <div className="h-56 flex items-center justify-center text-gray-400 font-semibold">
                        {t.noUsers || "No registration data."}
                      </div>
                    ) : (
                      <LineChart
                        data={regByDay}
                      ...
                      >
                        ...
                        <YAxis
                          ...
                          allowDecimals={false}
                          tick={{ fontSize: 12 }}
                          domain={[0, dataMax => (dataMax < 4 ? 4 : dataMax + 1)]}
                        />
                        <Tooltip
                          formatter={(value, name) => [`${value} ${t.registrations || 'Registrations'}`, t.date]}
                ...
                <div className="w-full mt-2 flex justify-end">
                  <span className="text-gray-400 text-xs">* {t.registrations || "Registrations"} {t.by?.toLowerCase() || "by"} day</span>
                </div>
              </div>
              {/* ----- END MODIFIED LINE CHART BLOCK ----- */}
            </div>
          </section>
        )}
        {/* USERS */}
